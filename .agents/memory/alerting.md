...
Operational alerts are limited to service health, audit integrity, orphan/lease anomalies, recovery failures, and sustained failure rate. `recovered` is an audit transition; the persisted current state returns to `healthy`.
